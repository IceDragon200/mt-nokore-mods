...
do
  ...
  --- @spec #trigger_on_player_respawn(player: PlayerRef): Boolean
  function ic:trigger_on_player_respawn(player)
    local player_name = player:get_player_name()
    local skip_reposition
    local result
    for callback_name, callback in pairs(self.m_on_player_respawn_cbs) do
      print("player_service", "on_player_respawn", player_name, "callback", callback_name)
      result = callback(player, timed_out)
      skip_reposition = skip_reposition or result
    end
    return skip_reposition
  end
  ...
  --- @spec #register_on_player_respawn(name: String, callback: Function/1): self
  function ic:register_on_player_respawn(name, callback)
    assert(type(name) == "string", "expected a registration name")
    assert(type(callback) == "function", "expected a callback")

    self.m_on_player_respawn_cbs[name] = callback
    return self
  end
  ...
  function ic:on_player_respawn(player)
    ...
    return self:trigger_on_player_respawn(player)
  end
