...
do
  ...
  function ic:register_domain(domain_name, def)
    ...
    if def.save_method ~= "apack" and
       ...
       def.save_method ~= "serialize" and
       def.save_method ~= "none" then
      error("expected domain save_method to be: \"apack\", \"marshall\", \"serialize\" or \"none\"")
    end
  ...
  function ic:on_player_join(player)
    ...
    for domain_name, domain_def in pairs(self.m_registered_domains) do
      ...
      elseif domain_def.save_method == "none" then
        --- Ain't nothing here
      end
  ...
  function ic:persist_domain(domain_name, domain)
    print("persisting domain", domain_name)
    ...
    if result then
      ...
      print("persisted domain", domain_name)
      return true
    else
