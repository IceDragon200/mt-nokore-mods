...
--- @namespace nokore_entity_walkover
...
mod.on_player_walk_in_cbs = {}

mod.on_player_standing_on_cbs = {}

mod.on_player_standing_in_cbs = {}
...
local function update_players(players, dtime, player_assigns)
  ...
  if mod.elapsed > STEP then
    ...
    for player_name,player in pairs(players) do
      ...
      walk_in_pos = Vector3.copy(pos)
      walk_in_pos.y = walk_in_pos.y + MINIMUM_DISTANCE
      walk_in_pos = Vector3.round(walk_in_pos, walk_in_pos)
      ...
      if assigns.last_moved_since then
        if assigns.last_moved_since <= 0 then
          ...
          node = minetest.get_node_or_nil(walk_in_pos)
          ...
          if node then
            ...
            for _, cb in pairs(mod.on_player_walk_in_cbs) do
              cb(walk_in_pos, node, player)
            end
          end
        elseif assigns.last_moved_since >= STANDING_THRESHOLD then
          ...
          if node then
            ...
            for _, cb in pairs(mod.on_player_standing_in_cbs) do
              cb(walk_in_pos, node, player, assigns.last_moved_since)
            end
          end
        end
      end
...
--- @spec &on_player_walk_in(name: String, callback: OnWalkoverCallback): void
function mod:on_player_walk_in(name, callback)
  self.on_player_walk_in_cbs[name] = callback
end
...
--- @spec &on_player_standing_in(name: String, callback: OnStandingOnCallback): void
function mod:on_player_standing_in(name, callback)
  self.on_player_standing_in_cbs[name] = callback
end
